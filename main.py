...
def getData(node:xml.dom.minidom.NodeList,convertValues:bool=True)->str:
    ...
    if node[0].nodeValue:
        ...
        if node[0].nodeName=="a1:Value" and convertValues:
                value=round(float(value)*2.54,0)
...
def extract_zip_file(file_path, extract_to):
    ...
    try:
        with zipfile.ZipFile(file_path, 'r') as zip_ref:
            with zip_ref.open("Groups.xml") as inner_zip_ref:
                DOMTree = xml.dom.minidom.parse(inner_zip_ref)
                libraryData = DOMTree.documentElement
    ...
    processedMaterialDicts=[]
    materials=libraryData.getElementsByTagName("a2:MaterialTemplate")
    
    for m in materials:
        materialDict={}
        for k,v in RELEVANT_TAGS.items():
            node=m.getElementsByTagName(k)
             
                #the value of relvant info dict is becomes the key for the output dict
            materialDict[v]=getData(node)
        processedMaterialDicts.append(materialDict)
    ic(processedMaterialDicts)
       
# Example usage
file_path = r"C:\Users\simon\Desktop\Material.library"
